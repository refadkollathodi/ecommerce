<!DOCTYPE html>
<html lang="en">
<?php
include ("conn.php");
session_start(); ?>

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="style.css?v=<?php echo time(); ?>">

    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css"
        integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js"
        integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN"
        crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.12.9/dist/umd/popper.min.js"
        integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q"
        crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/js/bootstrap.min.js"
        integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl"
        crossorigin="anonymous"></script>

</head>
<title>ecommerce</title>

<body>
    <div class="container-1">
        <!-- #navbar-->

        <div class="navbar">
            <div class="logo">
                <img src="./shopping-cart.png" alt="error" width="65px">
            </div>
            <nav>
                <ul>
                    <li><a href="#phones">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="bi bi-phone" viewBox="0 0 16 16">
                                <path
                                    d="M11 1a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zM5 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z" />
                                <path d="M8 14a1 1 0 1 0 0-2 1 1 0 0 0 0 2" />
                            </svg>
                            mobiles</a></li>
                    <li><a href="#tv"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="bi bi-tv" viewBox="0 0 16 16">
                                <path
                                    d="M2.5 13.5A.5.5 0 0 1 3 13h10a.5.5 0 0 1 0 1H3a.5.5 0 0 1-.5-.5M13.991 3l.024.001a1.5 1.5 0 0 1 .538.143.76.76 0 0 1 .302.254c.067.1.145.277.145.602v5.991l-.001.024a1.5 1.5 0 0 1-.143.538.76.76 0 0 1-.254.302c-.1.067-.277.145-.602.145H2.009l-.024-.001a1.5 1.5 0 0 1-.538-.143.76.76 0 0 1-.302-.254C1.078 10.502 1 10.325 1 10V4.009l.001-.024a1.5 1.5 0 0 1 .143-.538.76.76 0 0 1 .254-.302C1.498 3.078 1.675 3 2 3zM14 2H2C0 2 0 4 0 4v6c0 2 2 2 2 2h12c2 0 2-2 2-2V4c0-2-2-2-2-2" />
                            </svg> TV'S</a></li>
                    <li><a href="#acs"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                fill="currentColor" class="bi bi-earbuds" viewBox="0 0 16 16">
                                <path fill-rule="evenodd"
                                    d="M6.825 4.138c.596 2.141-.36 3.593-2.389 4.117a4.4 4.4 0 0 1-2.018.054c-.048-.01.9 2.778 1.522 4.61l.41 1.205a.52.52 0 0 1-.346.659l-.593.19a.55.55 0 0 1-.69-.34L.184 6.99c-.696-2.137.662-4.309 2.564-4.8 2.029-.523 3.402 0 4.076 1.948zm-.868 2.221c.43-.112.561-.993.292-1.969-.269-.975-.836-1.675-1.266-1.563s-.561.994-.292 1.969.836 1.675 1.266 1.563m3.218-2.221c-.596 2.141.36 3.593 2.389 4.117a4.4 4.4 0 0 0 2.018.054c.048-.01-.9 2.778-1.522 4.61l-.41 1.205a.52.52 0 0 0 .346.659l.593.19c.289.092.6-.06.69-.34l2.536-7.643c.696-2.137-.662-4.309-2.564-4.8-2.029-.523-3.402 0-4.076 1.948m.868 2.221c-.43-.112-.561-.993-.292-1.969.269-.975.836-1.675 1.266-1.563s.561.994.292 1.969-.836 1.675-1.266 1.563" />
                            </svg> Accessories</a></li>
                    <li><a href="cart.php">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                                class="bi bi-cart" viewBox="0 0 16 16">
                                <path
                                    d="M0 1.5A.5.5 0 0 1 .5 1H2a.5.5 0 0 1 .485.379L2.89 3H14.5a.5.5 0 0 1 .491.592l-1.5 8A.5.5 0 0 1 13 12H4a.5.5 0 0 1-.491-.408L2.01 3.607 1.61 2H.5a.5.5 0 0 1-.5-.5M3.102 4l1.313 7h8.17l1.313-7zM5 12a2 2 0 1 0 0 4 2 2 0 0 0 0-4m7 0a2 2 0 1 0 0 4 2 2 0 0 0 0-4m-7 1a1 1 0 1 1 0 2 1 1 0 0 1 0-2m7 0a1 1 0 1 1 0 2 1 1 0 0 1 0-2" />
                            </svg>
                            cart</a></li>
                    <?php
                    if (!isset($_SESSION['id'])) {
                        ?>
                        <li><a href="login.php"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="18"
                                    fill="currentColor" class="bi bi-person-up" viewBox="0 0 16 16">
                                    <path
                                        d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m.354-5.854 1.5 1.5a.5.5 0 0 1-.708.708L13 11.707V14.5a.5.5 0 0 1-1 0v-2.793l-.646.647a.5.5 0 0 1-.708-.708l1.5-1.5a.5.5 0 0 1 .708 0M11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4" />
                                    <path
                                        d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z" />
                                </svg> login
                            </a>
                        <?php } else { ?>
                        <li>
                            <p><svg xmlns="http://www.w3.org/2000/svg" width="22" height="18" fill="currentColor"
                                    class="bi bi-person-check" viewBox="0 0 16 16">
                                    <path
                                        d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m1.679-4.493-1.335 2.226a.75.75 0 0 1-1.174.144l-.774-.773a.5.5 0 0 1 .708-.708l.547.548 1.17-1.951a.5.5 0 1 1 .858.514M11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4" />
                                    <path
                                        d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z" />
                                </svg> hi,<?php echo $_SESSION['username']; ?></p>
                        </li>
                    <?php } ?>

                    <li>
                        <a href="logout.php"><svg xmlns="http://www.w3.org/2000/svg" width="22" height="18"
                                fill="currentColor" class="bi bi-person-down" viewBox="0 0 16 16">
                                <path
                                    d="M12.5 9a3.5 3.5 0 1 1 0 7 3.5 3.5 0 0 1 0-7m.354 5.854 1.5-1.5a.5.5 0 0 0-.708-.708l-.646.647V10.5a.5.5 0 0 0-1 0v2.793l-.646-.647a.5.5 0 0 0-.708.708l1.5 1.5a.5.5 0 0 0 .708 0M11 5a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4" />
                                <path
                                    d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z" />
                            </svg> logout</a>
                    </li>
                </ul>
            </nav>
        </div>

        <!-- #fronthome -->

        <div class="row">
            <div class="col-2">
                <h1>BUY NOW OR CRY LATER</h1>
                <p>mobiles and Accessories</p>
            </div>
            <div class="col-2">
                <img src="./index-shopping.png" alt="">
            </div>
        </div>
    </div>
    </div>
    <!-- deals -->

    <div class="categories">
        <br>
        <h1 id="mobile-h1">OUR PRODUCTS</h1>
        <div class="small">
            <div class="row">
                <div class="col-3"><img src="realme9.jpeg" alt=""></div>
                <div class="col-3"><img src="iphone15.jpg" alt=""></div>
                <div class="col-3"><img src="redmi12.jpeg" alt=""></div>
            </div>
        </div>

    </div>
    <!-- #products -->
    <?php
    include ("conn.php");
    $sql = "SELECT * FROM products WHERE category = 'mobile'";
    $result = mysqli_query($conn, $sql);
    ?>
    <!-- phones -->
    <section class="products" id="phones">
        <h1 id="mobile-h1">mobiles</h1>
        <div class="all-products">
            <?php
            while ($row = mysqli_fetch_assoc($result)) {
                ?>
                <div class="product">
                    <img src="<?php echo $row['productimage']; ?>" alt="">
                    <div class="product-info">

                        <h4 class="product-title">
                            <?php echo $row['product_name']; ?>
                        </h4>
                        <p class="product-price"><?php echo $row['price']; ?></p>
                        <a href="checkout.php?edited=<?php echo $row['product_id']; ?>" class="product-btn">buy now</a>
                        <a href="add_to_cart.php?edited=<?php echo $row['product_id']; ?>" class="product-btn">add to
                            cart</a>

                    </div>
                </div>
                <?php
            }
            ;
            ?>
        </div>

    </section>

    <?php
    include ("conn.php");
    $sql = "SELECT * FROM products WHERE category = 'tv'";
    $result = mysqli_query($conn, $sql);
    ?>
    <!-- tv's -->

    <section class="products" id="tv">
        <h1 id="mobile-h1">TV'S</h1>
        <div class="all-products">
            <?php
            while ($row = mysqli_fetch_assoc($result)) {
                ?>
                <div class="product">
                    <img src="<?php echo $row['productimage']; ?>" alt="">
                    <div class="product-info">

                        <h4 class="product-title">
                            <?php echo $row['product_name']; ?>
                        </h4>
                        <p class="product-price"><?php echo $row['price']; ?></p>
                        <a href="checkout.php?edited=<?php echo $row['product_id']; ?>" class="product-btn">buy now</a>
                        <a href="add_to_cart.php?edited=<?php echo $row['product_id']; ?>" class="product-btn">add to
                            cart</a>

                    </div>
                </div>
                <?php
            }
            ;
            ?>
        </div>

        <!-- earphone -->

        <?php
        include ("conn.php");
        $sql = "SELECT * FROM products WHERE category = 'earphone'";
        $result = mysqli_query($conn, $sql);
        ?>

        <section class="products" id="acs">
            <h1 id="mobile-h1">earphones</h1>
            <div class="all-products">
                <?php
                while ($row = mysqli_fetch_assoc($result)) {
                    ?>
                    <div class="product">
                        <img src="<?php echo $row['productimage']; ?>" alt="">
                        <div class="product-info">

                            <h4 class="product-title">
                                <?php echo $row['product_name']; ?>
                            </h4>
                            <p class="product-price"><?php echo $row['price']; ?></p>
                            <a href="checkout.php?edited=<?php echo $row['product_id']; ?>" class="product-btn">buy now</a>
                            <a href="add_to_cart.php?edited=<?php echo $row['product_id']; ?>" class="product-btn">add to
                                cart</a>
                        </div>
                    </div>
                    <?php
                }
                ;
                ?>
            </div>


        </section>

        </div>

</body>

</html>